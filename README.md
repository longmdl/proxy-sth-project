...
3. **Mô hình Dữ liệu (PostgreSQL)**:
   - Các bảng dữ liệu như `EbUser`, `EkycSession`, và `NfcTicket` được cấu hình lưu trữ qua JPA/Hibernate xuống PostgreSQL.

## Hướng dẫn Chạy ứng dụng

Yêu cầu hệ thống:
- Docker & Docker Compose (Khuyên dùng)
- *Hoặc*: Java 17+, Maven, và PostgreSQL cục bộ.

### Cách 1: Sử dụng Docker Compose (Đơn giản nhất)
Bạn có thể khởi động cả Database và Ứng dụng chỉ với 1 lệnh:
```bash
docker-compose up --build
```
Ứng dụng sẽ tự động được build và khởi động ở cổng 8080. CSDL Postgres chạy ở cổng 5432.

### Cách 2: Chạy thủ công bằng Maven
1. Đảm bảo bạn đã có sẵn PostgreSQL chạy ở cổng `5432` với db `proxydb` (hoặc sửa đổi biến môi trường).
2. Khởi động server bằng Maven:
   ```bash
...
- Bạn có thể **đăng ký người dùng mới** ở cột Người hỗ trợ.
- Một số SĐT Helper mặc định vẫn hoạt động nếu bạn không muốn tạo mới (nếu insert sẵn vào DB hoặc tự tạo qua Form).
